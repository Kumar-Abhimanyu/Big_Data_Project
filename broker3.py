...
def after_request(response):
    ...
    f = open(os.getcwd() + '\\Data\\Broker3\\' + 'log.txt', 'a')
...
def topic_data():
    ...
    if (I_am_leader()):
        ...
        if result1 == 0:
            requests.post('http://localhost:5000/topic_data', json = data_to_send, headers = headers)
        sock1.close()
    ...
    topic_folder_path = os.getcwd() + '\\Data\\Broker3\\' + topic
...
def delete_topic():
    ...
    if (I_am_leader()):
        ...
        result1 = sock1.connect_ex(('127.0.0.1', 5000))
        if result1 == 0:
            requests.post('http://localhost:5000/delete_topic', json = data_to_send, headers = headers)
        sock1.close()
        sock2 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        result2 = sock2.connect_ex(('127.0.0.1', 5001))
        if result2 == 0:
    ...
    topic_folder_path = os.getcwd() + '\\Data\\Broker3\\' + topic_to_delete
    if (not os.path.isdir(topic_folder_path)):
...
def register_consumer():
    ...
    folder_path = os.getcwd() + '\\Data\\Broker3\\' + topic
    if (os.path.isdir(folder_path)):
        ...
        result2 = sock2.connect_ex(('127.0.0.1', 5001))
        if result2 == 0:
            requests.post('http://localhost:5001/create_new_topic', json = data_to_send, headers = headers)
        sock2.close()
